...
const emptyGame = () => ({
  ...
  turn: 1, winner: 0, winLine: null, moves: 0, round: 1,
  seats: { 1: null, 2: null },       // {token,name,online}
});

export class GameRoom {
  constructor(ctx) { this.ctx = ctx; this.game = null; }
  ...
  stateFor(g, role) {
    return { t: 'state', board: g.board.join(''), turn: g.turn, winner: g.winner, winLine: g.winLine,
             round: g.round, role, seats: this.seatsPub(g) };
  }
  ...
  async webSocketMessage(ws, raw) {
    ...
    if (m.t === 'join') {
      ...
      if (!role) for (const s of [1, 2]) if (!role && (!g.seats[s] || !g.seats[s].online)) { // claim empty/offline seat
        g.seats[s] = { token, name, online: true }; role = s;
      }
      if (role) { g.seats[role].online = true; g.seats[role].name = name; }
      ws.serializeAttachment({ token, name, role });
      await this.save();
      ws.send(JSON.stringify(this.stateFor(g, role)));
      this.bcast(this.roster(g));
      this.bcast({ t: 'sys', text: `${name} 입장 ${role ? (role === 1 ? '(흑)' : '(백)') : '(관전)'}` }, ws);
      return;
    }
    ...
    if (m.t === 'restart') {                     // players only, after a finished game (or on an untouched board)
      if (att.role !== 1 && att.role !== 2) return;
      if (!g.winner && g.moves > 0) return;
  ...
  async webSocketClose(ws) {
    ...
    if (!att) return;
    const g = await this.load();
    if (att.role && g.seats[att.role] && g.seats[att.role].token === att.token) {
      let still = false;                          // same player may have another tab open
      for (const w of this.ctx.getWebSockets()) {
        if (w === ws) continue;
        const a = w.deserializeAttachment(); if (a && a.token === att.token) { still = true; break; }
      }
      if (!still) { g.seats[att.role].online = false; await this.save(); }
    }
    this.bcast(this.roster(g));
    if (att.name) this.bcast({ t: 'sys', text: `${att.name} 퇴장` });
  }
